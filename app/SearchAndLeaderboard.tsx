...
import { getSearchProfiles, getProfileSearchCounts, updateProfileSearchCount } from './utils/profileStorage';
...
const SearchAndLeaderboard: React.FC<SearchAndLeaderboardProps> = ({ onProfileSelect, connectedWallet }) => {
  ...
  const loadProfiles = () => {
    const savedProfiles = getSearchProfiles();
    ...
    const profilesWithCounts = savedProfiles.map(profile => ({
      ...
      avatarUrl: (profile.avatarUrl || profile.icon || '/monad_logo.ico') as string,
                ...
                {filteredProfiles.length > 0 ? (
                  filteredProfiles.map((profile, index) => (
                    <div 
                      ...
                      title={`Click to select profile | Search count: ${profile.searchCount}`}
                    >
                ...
                {leaderboardProfiles.length > 0 ? (
                  leaderboardProfiles.map((profile, index) => (
                    <div 
                      ...
                      onClick={() => handleProfileClick(profile)}
                      title={`Rank #${index + 1} | ${profile.searchCount} searches`}
                    >
